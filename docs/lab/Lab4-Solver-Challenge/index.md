...
实验最终的测评环境是 Slurm 上的 **`solver2` 分区，将提供 3 个节点** (`M602-M604`)，每个节点最多使用 52 个物理核心 (2 × Intel(R) Xeon(R) Gold 5320 CPU @ 2.20GHz)，以及 503 GiB 内存，每个任务的**时间上限为 2 分钟**。

如果你希望更多的时间用于调试，可以**在 `solver1` 分区申请单台机器**，**时限为 15 分钟**。

你可以使用允许范围内的任意节点数和核心数的组合完成本次实验，请在提交的 SBATCH 脚本中调整对应参数，并在最终报告中加以说明。
