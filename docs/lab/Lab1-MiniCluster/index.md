...
- 软件安装：
    - 选择一：下载 OpenMPI、BLAS 和 HPL 的源代码并编译安装。
    - 选择二：使用 Spack 安装 OpenMPI、BLAS 和 HPL。
    - 选择三：使用你喜欢的方式安装 OpenMPI、BLAS 和 HPL。

!!! tip "任选一种你喜欢的软件安装方式即可"

    我们不对软件安装的方式做硬性限制，你可以使用源码编译、包管理器安装、spack 安装等任何方式安装软件, 但是不允许对HPL的算法进行修改。
