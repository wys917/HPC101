...
!!! bug "选课报名已经截止"
    ...
    本次 HPC 101 超算短学期报名于 2025 年 5 月 30 日 24:00 正式截止。我们一共收到了【126】份报名表，相比去年增长了【186%】。报名表的提交时间分布统计信息如下：

    ![2025_statistics](index.assets/2025_statistics.webp)

    最后一封邮件于【5 月 30 日 24:00:45】送达邮箱。我们在最后一天收到的报名表数量超过之前的总和，可以看出大家都是 DDL 战神。

    接下来两天还会有几场【一起聊聊】，希望参与的同学都能顺利进入课程。

    我们将在 6 月 2 日将名单提交到教学科，课程应当在 3 日内进入大家的课表。如有异常请及时向我们反馈。

!!! quote inline end ""

    ![2025 QQ Group](index.assets/2025_qqgroup.webp)

欢迎扫码进入 2025 HPC101 课程交流群了解更多信息：
...
请有意向选课的同学完成 Lab 1 (MiniCluster) 实验并提交实验报告，如果你实在没有时间完成实验，也可以提交个人简介。

同时我们也提供了 Lab 0 (Linux Crash Course) 实验，帮助你快速上手 Linux 系统，你可以根据自身情况**选择性完成**，并将实验结果一并提交。

请将实验报告或个人简介发送到邮箱：

!!! warning "请注意邮件格式"

    我们使用自动化流程处理报名邮件，因此请务必按照以下格式发送邮件。因为邮件格式不正确、缺少信息等原因导致无法联络的同学，后果自负。

- **邮箱**：`mail@zjusct.io`
- **邮件标题**：`【2025 短学期报名表】姓名 学号`，例：`【2025 短学期报名表】张三 3220100001`
- **邮件内容**：个人联系方式（邮箱、电话、QQ、微信等）
- **邮件附件**：Lab 1 实验报告或个人简介**至少一个**
- **截止时间**：5 月 30 日晚 23:59
- **一起聊聊**：我们希望同每一位希望修读该课程的同学聊聊天，请不要将该环节当作面试。**请在 [ZJUSCT-HPC101-2025 - Schej](https://schej.it/e/E8Efee) 上选择你方便的时间段，并在邮件内说明**，方便我们安排时间。将以群聊的形式进行，每场约半小时。
...
- 上课时间：上午 8:30 - 11:30，下午 14:00-17:00
- 上课地点：待定
...
| 6.28  | HPC 中的计算机系统 - 2       | 何水兵老师/宋浩喆    |
| 6.29  | 休息（HPC-PL）             | /               |
| 6.30  | 向量化并行计算基础         | 陈宏哲            |
| 7.1  | Cuda C 编程基础            | 王则可老师/陈楷骐 |
| 7.2  | （上午）OpenMP/MPI 并行计算基础<br>（下午）C++ 并行编程    | 李晨潇<br>朱宝林     |
| 7.3  | 高性能计算高级话题         | 王则可老师      |
| 7.4 | 休息                       | /               |
| 7.5 | 性能分析技术基础 Profiling   | 朱宝林          |
| 7.6 | 机器学习基础             | 林熙        |
| 7.7 | 机器学习高级话题           | 张寅老师        |
...
!!! question "硕博可以选吗？"
    ...
    笔者不清楚硕博的选课流程，但往年确实有硕博通过选课系统选上了我们的课。
...
!!! question "可以旁听吗？"
    ...
    如需申请集群账户以便进行实验和练习，请发送邮件至 `mail@zjusct.io`，邮件内容需包含以下信息：

    - 姓名、学号、学院/专业
    - 个人联系方式（邮箱、电话、QQ、微信等）
    - 旁听理由（简要说明希望旁听课程及申请集群账户的原因）

    由于集群计算资源有限，账户申请将根据实际情况进行审核，**不保证所有申请都能通过**。请理解资源分配的限制，感谢支持！
